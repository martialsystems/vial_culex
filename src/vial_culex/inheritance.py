...
"""Meiosis and mutation. Founder IDs follow allelic lineage."""
...
from vial_culex.fitness import Phenotype, clutch_sizes
from vial_culex.genome import THIEF_PATH, Pop, clip_qtl, empty_pop
...
def meiosis_mutate(
    ...
    n_rare = int(rare.sum())
    if n_rare:
        host = np.asarray(THIEF_PATH, dtype=np.int64)
    ...
    return eggs, np.repeat(np.arange(fi.size), clutch), clutch
